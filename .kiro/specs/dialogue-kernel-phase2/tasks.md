...
- [x] 1. Expand dialogue type definitions (`core/dialogue/types.h`)
  - [x] 1.1 Populate Phase 1 stub event structs with real payloads
  ...
  - [x] 1.2 Add new event types and enums
  ...
  - [x] 1.3 Expand DialogueState with Phase 2 fields
  ...
  - [x] 1.4 Add new effect types to DialogueEffect variant
...
- [x] 2. Implement TimerBook (`core/dialogue/timer_book.h` and `core/dialogue/timer_book.cpp`)
  - [x] 2.1 Create `core/dialogue/timer_book.h` with class declaration
  ...
  - [x] 2.2 Implement `core/dialogue/timer_book.cpp`
...
- [x] 3. Checkpoint — Verify expanded types and TimerBook compile
...
- [x] 4. Implement Phase 2 reducer handlers (`core/dialogue/default_reducer.h` and `core/dialogue/default_reducer.cpp`)
  - [x] 4.1 Update `default_reducer.h` — add Phase 2 handler declarations
  ...
  - [x] 4.2 Implement `HandleUserMessage` — normal path (cooldown == kNone)
    ...
    - When `cooldown == kNone` and `deliberation == kAwaitingTurnTrigger` (superseding message): emit `CancelTurnTriggerClassification` for the old request, then process the new message normally — emit `RecordMemory` + `StartTurnTriggerClassification` with a fresh `pending_turn_trigger_id = ++next_turn_trigger_id`. The stale classification result will be rejected by obs_id mismatch.
  ...
  - [x] 4.3 Implement `HandleTurnTriggerClassified`
  ...
  - [x] 4.4 Implement `HandleLlmCompleted`
  ...
  - [x] 4.7 Implement `HandleToolCallTimeout`
  ...
  - [x] 4.8 Update `HandleInterrupt` — add ScheduleTimer, cancel in-flight tool work, and RecordInterruptMemory
    ...
    - NOTE: `kAwaitingTurnTrigger` is NOT reachable via InterruptRequested — it corresponds to Controller FSM state kListening which is non-interruptible. Turn-trigger cancellation during kAwaitingTurnTrigger is handled by superseding UserMessageReceived in task 4.2.
    - Set `deliberation = kIdle` (or keep as appropriate for debounce)
    - _Requirements: 11.1, 14.1, 14.2_
  - [x] 4.9 Implement `HandleTimerExpired`
  ...
  - [x] 4.10 Implement `HandleSystemEvent`
  ...
  - [x] 4.11 Implement `HandleContinuation`
    - When `deliberation == kIdle` or `kThinking`: emit `StartLlm`, set `deliberation = kThinking`
    - _Requirements: 13.1_
  - [x] 4.12 Update `Reduce` dispatch — wire all new handlers via `std::visit`
...
- [x] 5. Checkpoint — Verify expanded reducer compiles
  - Build the project and verify no compile errors
  - Ensure existing Phase 1 reducer tests still pass

- [x] 6. Write unit tests for Phase 2 reducer handlers
  - [x] 6.1 Create `tests/agent/dialogue_reducer_phase2_test.cpp` with unit tests
    ...
    - Test `HandleUserMessage` superseding during kAwaitingTurnTrigger: verify CancelTurnTriggerClassification in effects, new RecordMemory + StartTurnTriggerClassification with fresh id, pending_turn_trigger_id changed
    ...
    - _Requirements: 19.1_
  - [x] 6.2 Create `tests/agent/timer_book_test.cpp` with unit tests
...
- [x] 7. Write property tests for Phase 2 reducer and TimerBook
  - [x] 7.1 Write property test: Reducer purity — extended (Property 1)
  ...
  - [x] 7.2 Write property test: State consistency invariants (Property 2)
  ...
  - [x] 7.3 Write property test: TimerBook schedule-then-pop round trip (Property 3)
  ...
  - [x] 7.4 Write property test: TimerBook cancel exclusion (Property 4)
  ...
  - [x] 7.5 Write property test: Normal user message records memory and triggers turn evaluation (Property 5)
  ...
  - [x] 7.6 Write property test: Turn-trigger verdict routing (Property 7)
  ...
  - [x] 7.7 Write property test: Stale turn-trigger rejection (Property 8)
    - For any `DialogueState` and any `TurnTriggerClassified` with non-matching `obs_id`, state unchanged with no effects
    - Run minimum 100 iterations
    - **Validates: Requirements 6.3**
  - [x] 7.8 Write property test: LLM completion token and turn accounting (Property 9)
  ...
  - [x] 7.9 Write property test: LLM result routing by action type (Property 10)
  ...
  - [x] 7.10 Write property test: Tool result collection and continuation (Property 12)
  ...
  - [x] 7.11 Write property test: Interrupt cancels in-flight tool work (Property 14)
    - Precondition: Controller FSM is in an interruptible state (use RC_PRE to constrain deliberation to kThinking or kAwaitingToolResults — these are the deliberation phases that correspond to interruptible Controller FSM states. kAwaitingTurnTrigger is NOT interruptible.)
    - For any valid `InterruptRequested`: effects contain ScheduleTimer{kDebounce} and RecordInterruptMemory; if kAwaitingToolResults → CancelTimer
    - Run minimum 100 iterations
    - **Validates: Requirements 11.1, 14.1, 14.2**
  - [x] 7.12 Write property test: Activity tracking on message events (Property 16)
  ...
  - [x] 7.13 Write property test: kStoreOnly does not start an assistant turn (Property 17)
  ...
  - [x] 7.14 Write property test: Timer id reuse after cancel (Property 18)
  ...
  - [x] 7.15 Write property test: Interrupt always records interrupt memory (Property 19)
    - Precondition: deliberation is kThinking or kAwaitingToolResults (corresponding to interruptible Controller FSM states — kAwaitingTurnTrigger is NOT interruptible)
    - For any such `DialogueState` and any `InterruptRequested` event, effects SHALL contain `RecordInterruptMemory`
    - Run minimum 100 iterations
    - **Validates: Requirements 14.2**
  - [x] 7.16 Write property test: Superseding UserMessageReceived cancels pending turn-trigger (Property 20)
    - For any `DialogueState` where `deliberation == kAwaitingTurnTrigger` and any `UserMessageReceived`, effects SHALL contain `CancelTurnTriggerClassification` and `StartTurnTriggerClassification` (with a new obs_id), and `next_state.pending_turn_trigger_id` SHALL differ from the original `state.pending_turn_trigger_id`
    - Run minimum 100 iterations
    - **Validates: Requirements 14.3**

- [x] 8. Update test build system
  - [x] 8.1 Add Phase 2 test files to `tests/agent/CMakeLists.txt`
...
- [x] 9. Checkpoint — Verify all reducer and TimerBook tests pass
...
- [x] 10. Wire Phase 2 reducer into Controller
  - [x] 10.1 Update `Controller::ApplyDialogueDecision` — add Phase 2 effect handlers
    ...
    - _Requirements: 15.4_
  - [x] 10.2 Add `TimerBook` member to Controller and integrate into RunLoop
  ...
  - [x] 10.3 Replace normal user message inline logic with reducer calls
    ...
    - _Requirements: 15.3_
  - [x] 10.4 Replace LLM completion inline logic with reducer calls
    ...
    - _Requirements: 15.3_
  - [x] 10.5 Replace LLM failure inline logic with reducer calls
    - After LLM fails in HandleThinking: construct `LlmFailed{reason, now}`, call `Reduce`, apply decision
    - _Requirements: 15.3_
  - [x] 10.6 Replace tool result inline logic with reducer calls
    ...
    - _Requirements: 15.3_
  - [x] 10.7 Replace tool call timeout inline logic with reducer calls
    ...
    - _Requirements: 15.3_
  - [x] 10.8 Replace system event inline logic with reducer calls
    - Replace the `obs.type == kSystemEvent` block with: construct `SystemEventReceived{obs, now}`, call `Reduce`, apply decision
    - _Requirements: 15.3_
  - [x] 10.9 Remove `SyncBudgetToDialogueState()` and redundant Controller fields
...
- [x] 11. Checkpoint — Verify existing tests still pass after Controller rewiring
...
- [x] 12. Write controller-level regression tests for Phase 2 paths
  - [x] 12.1 Add regression tests to `tests/agent/controller_test.cpp` (or a new file)
    ...
    - Test: superseding message during turn-trigger evaluation → old evaluation cancelled, new message recorded + new classification started with fresh id, stale result from old evaluation rejected
    - Test: timer id reuse — Schedule("debounce") → Cancel("debounce") → Schedule("debounce") → only second timer fires
    - Test: interrupt always records interrupt memory — verify "Turn interrupted" entry in committed history when interrupt arrives in interruptible states (kThinking, kAwaitingTurnTrigger, kAwaitingToolResults); verify interrupt is no-op in non-interruptible states (kIdle, kListening) per existing Controller::Interrupt() contract
